...
import os
# a module for beautiful command-line interface
...
# use docopt for beautiful command-line interface
...
# unityEnvironment in /unityaganets/environment.py
...
# create model path if not
if not os.path.exists(model_path):
    os.makedirs(model_path)
# create summary path if not
...
with tf.Session() as sess:
    ...
    info               = env.reset(train_mode = train_model,
                                   progress   = get_progress())[brain_name]
    trainer            = Trainer(ppo_model,
                                 sess,
                                 info,
                                 is_continuous,
                                 use_observations,
...
env.close()
graph_name = (env_name.strip().replace('.app', '').replace('.exe', '').replace('.x86_64', '').replace('.x86', ''))
